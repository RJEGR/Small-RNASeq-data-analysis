...
DataVizsum <- DataViz %>%
  ...
  mutate(Label = as.character(COG_name), row_number = min_rank(Label)) %>% 
  mutate(Label = factor(paste(Label, row_number, sep = "__"), 
    levels = rev(paste(Label, row_number, sep = "__")))) 
...
levelshpf <- DataVizsum %>% ungroup %>% 
  # filter(Contrast %in% "24 hpf") %>% 
  distinct(COG_name, Contrast) %>%
  mutate(COG_name = factor(paste(COG_name, Contrast, sep = "__"))) %>% pull(COG_name)
...
DB %>% 
  ...
  # geom_segment(aes(xend = 3, yend = COG_name), linewidth = 0.5, color="#E7DFD5", linetype="dashed") +
