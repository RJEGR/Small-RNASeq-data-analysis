...
alignment_details %>%
  ...
  theme(strip.background = element_rect(fill = 'white', color = 'white'),
    ...
    axis.text.x = element_text(size = 12, angle = 0, hjust = 1, vjust = 1)) -> ps

ggsave(ps, filename = 'ALIGNMENT_DETAILS_FACET.png', path = path, width = 6.7, height = 8, dpi = 300, device = png)
...
p1 <- alignment_details %>%
  group_by(mapping_type,read_length, readfile) %>%
  summarise(n = sum(count)) %>%
  dplyr::mutate(mapping_type = dplyr::recode_factor(mapping_type, !!!recode_to)) %>%
  dplyr::mutate(read_length = factor(read_length, levels = read_lengthL)) %>%
  group_by(read_length, readfile) %>% mutate(frac = n/sum(n)) %>%
...
ps <- p1/p2  + patchwork::plot_layout(heights = c(1.2,0.7))

ggsave(ps, filename = 'ALIGNMENT_DETAILS_HEATMAP.png', path = path, width = 12, height = 6.5, dpi = 300, device = png)
